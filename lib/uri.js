 ...
 */
URI.Protocols = ['bitcoincash', 'bchtest', 'bchreg'];

URI.parse = function(uri) {
  var info = URL.parse(uri, true);

  info.protocol = info.protocol.slice(0, -1);
  if (URI.Protocols.indexOf(info.protocol) < 0) {
...
URI.prototype._parseAmount = function(amount) {
  ...
  return Unit.fromBCH(amount).toSatoshis();
};
...
URI.prototype.toString = function() {
  var query = {};
  if (this.amount) {
    query.amount = Unit.fromSatoshis(this.amount).toBCH();
  }
  if (this.message) {
