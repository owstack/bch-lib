...
describe('Unit', function() {
  ...
  it('has property accesors "BCC", "mBCC", "uBCC", "bits", and "satoshis"', function() {
    var unit = new Unit(1.2, 'BCC');
    unit.BCC.should.equal(1.2);
    unit.mBCC.should.equal(1200);
    unit.uBCC.should.equal(1200000);
  ...
  it('a string amount is allowed', function() {
    ...
    unit = Unit.fromBCC('1.00001');
    unit.BCC.should.equal(1.00001);

    unit = Unit.fromMilis('1.00001');
    unit.mBCC.should.equal(1.00001);
    ...
    unit.BCC.should.equal(0.12285714);
  });
  ...
  it('exposes unit codes', function() {
    should.exist(Unit.BCC);
    Unit.BCC.should.equal('BCC');

    should.exist(Unit.mBCC);
    Unit.mBCC.should.equal('mBCC');
  ...
  it('exposes a method that converts to different units', function() {
    var unit = new Unit(1.3, 'BCC');
    unit.to(Unit.BCC).should.equal(unit.BCC);
    unit.to(Unit.mBCC).should.equal(unit.mBCC);
  ...
  it('exposes shorthand conversion methods', function() {
    var unit = new Unit(1.3, 'BCC');
    unit.toBCC().should.equal(unit.BCC);
    unit.toMilis().should.equal(unit.mBCC);
    unit.toMillis().should.equal(unit.mBCC);
  ...
  it('can be imported and exported from/to JSON', function() {
    var json = JSON.stringify({amount:1.3, code:'BCC'});
  ...
  it('fails when the unit is not recognized', function() {
    ...
    expect(function() {
      return new Unit(100, 'BCC').to('USD');
    }).to.throw(errors.Unit.UnknownCode);
  });
  ...
  it('fails when the exchange rate is invalid', function() {
    ...
    expect(function() {
      return new Unit(100, 'BCC').atRate(-123);
    }).to.throw(errors.Unit.InvalidRate);
  });
